...
struct LCA {
  ...
  std::vector<std::vector<int>>
      parent;             // parent[i][k]:2^k-th parent of i-th node
  ...
  void dfs(const std::vector<std::vector<int>> &graph, int v, int p,
           int d) { // graph,current node,previous node,depth
    depth[v] = d;
    parent[v][0] = p;
    for (int i = 0; i < (int)graph[v].size(); ++i) {
      if (graph[v][i] != p)
        dfs(graph, graph[v][i], v, d + 1);
    }
  }
