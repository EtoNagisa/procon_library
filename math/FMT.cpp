...
static std::vector<long long> convolution_mod(const std::vector<long long> &p,
                                              const std::vector<long long> &q,
                                              long long m) {
  constexpr long long m1 = 1224736769ll, m2 = 998244353ll;
  ...
  for (int i = 0; i < sz; ++i) {
    long long x0 = res1[i], x1 = (res2[i] - x0 + m2) * minv % m2;
    ret[i] = (x0 + x1 * m1) % m;
  }
  return ret;
}
